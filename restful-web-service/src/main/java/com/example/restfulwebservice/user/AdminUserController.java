...
@RestController
@RequestMapping("/admin")       // /admin 요청이 default가 붙는다. (prefix느낌)
public class AdminUserController {
    ...
    public MappingJacksonValue retriveAllUsers() {
        List<UserV1> users = service.findAll();
    ...
    public MappingJacksonValue retriveUserV2(@PathVariable int id){
        ...
        // User -> UserV2
        // BeanUtils 는 Bean들간의 작업을 수행해줌.
        UserV2 userV2 = new UserV2();
        BeanUtils.copyProperties(user, userV2); // id, name, joinDate, password, ssn를 복사함(속성 복사)
        userV2.setGrade("VIP");
